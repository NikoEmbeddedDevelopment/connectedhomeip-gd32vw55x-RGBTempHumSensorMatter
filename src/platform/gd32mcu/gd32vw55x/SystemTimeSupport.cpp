 ...
 *          time/clock functions that are suitable for use on the GD32VW55x platform.
 */
/* this file behaves like a config.h, comes first */
#include <platform/internal/CHIPDeviceLayerInternal.h>

#include <platform/gd32mcu/gd32vw55x/SystemTimeSupport.h>
...
CHIP_ERROR ClockImpl::GetClock_RealTime(Clock::Microseconds64 & curTime)
{
    ...
    printf("GetClock_RealTime \r\n");
    return CHIP_ERROR_UNSUPPORTED_CHIP_FEATURE;

#if 0
    uint32_t sec, usec;

    get_time(SINCE_EPOCH, &sec, &usec);

    if (sec < CHIP_SYSTEM_CONFIG_VALID_REAL_TIME_THRESHOLD)
    {
        return CHIP_ERROR_REAL_TIME_NOT_SYNCED;
    }
    static_assert(CHIP_SYSTEM_CONFIG_VALID_REAL_TIME_THRESHOLD >= 0, "We might be letting through negative tv_sec values!");
    curTime = Microseconds64((static_cast<uint64_t>(sec) * UINT64_C(1000000)) + static_cast<uint64_t>(usec));
