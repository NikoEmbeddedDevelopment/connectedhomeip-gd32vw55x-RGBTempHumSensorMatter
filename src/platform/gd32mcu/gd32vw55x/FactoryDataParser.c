...
bool ParseFactoryData(uint8_t * buffer, uint16_t bufferSize, struct FactoryData * factoryData)
{
    ...
    while (res)
    {
        ...
        else if (strncmp("dis_mode", (const char *) currentString.value, currentString.len) == 0)
        {
            /* discovery mode, (0:WiFi-SoftAP, 1:BLE, 2:On-network. Default is BLE) */
            res                               = res && uint16_decode(states, &factoryData->dis_mode);
            factoryData->dismodePresent = res;
        }
        ...
        else if (strncmp("date", (const char *) currentString.value, currentString.len) == 0)
        {
            ...
            {
                printf("Parsing error - wrong date format");
            }
        }
