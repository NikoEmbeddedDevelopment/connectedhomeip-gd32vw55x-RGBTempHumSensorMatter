...
#include "gd32vw55x_route_hook.h"
#include "gd32vw55x_route_table.h"
...
typedef struct gd32vw55x_route_hook_t
{
    struct netif * netif;
    struct raw_pcb * pcb;
    struct gd32vw55x_route_hook_t * next;
} gd32vw55x_route_hook_t;
...
static gd32vw55x_route_hook_t * s_hooks;
...
static void ra_recv_handler(struct netif * netif, const uint8_t * icmp_payload, uint16_t payload_len, const ip6_addr_t * src_addr)
{
    ...
    while (payload_len >= 2)
    {
            ...
            payload_len >= opt_len)
        {
            ...
            GD32VW55xLog("Received RIO\n");

            if (rio_data_len >= prefix_len_bytes)
            {
                ip6_addr_t prefix;
                gd32vw55x_route_entry_t route;
                ...
                GD32VW55xLog("prefix %s lifetime %u\n", ip6addr_ntoa(&prefix), route.lifetime_seconds);
                if (gd32vw55x_route_table_add_route_entry(&route) == NULL)
                {
                    GD32VW55xLog("Failed to add route table entry\n");
                }
                else
                {
                    GD32VW55xLog("Added entry to route table\n");
                }
            }
        }
...
static uint8_t icmp6_raw_recv_handler(void * arg, struct raw_pcb * pcb, struct pbuf * p, const ip_addr_t * addr)
{
    ...
    ip6_addr_t src;
    ip6_addr_t dest;
    gd32vw55x_route_hook_t * hook = (gd32vw55x_route_hook_t *) arg;
    ...
    if (p->tot_len != p->len)
    {
        GD32VW55xLog("Ignore segmented ICMP packet\n");
        return 0;
    }
    if (p->tot_len <= sizeof(struct ip6_hdr) + sizeof(struct icmp6_hdr))
    {
        GD32VW55xLog("Ignore invalid ICMP packet\n");
        return 0;
    }
...
int8_t gd32vw55x_route_hook_init()
{
    struct netif * lwip_netif = (struct netif *)vif_idx_to_net_if(WIFI_VIF_INDEX_DEFAULT);
    ip_addr_t router_group    = IPADDR6_INIT_HOST(0xFF020000, 0, 0, 0x02);
    gd32vw55x_route_hook_t * hook = NULL;
    uint8_t ret               = 0;

    if (lwip_netif == NULL)
    {
        GD32VW55xLog("Invalid network interface\n");
        return -1;
    }

    for (gd32vw55x_route_hook_t * iter = s_hooks; iter != NULL; iter++)
    {
        if (iter->netif == lwip_netif)
        {
            GD32VW55xLog("Hook already installed on netif, skip...\n");
            return 0;
        }
    }

    hook = (gd32vw55x_route_hook_t *) malloc(sizeof(gd32vw55x_route_hook_t));
    if (hook == NULL)
    {
        GD32VW55xLog("Cannot allocate hook\n");
        return -1;
    }

    if (mld6_joingroup_netif(lwip_netif, ip_2_ip6(&router_group)) != ERR_OK)
    {
        GD32VW55xLog("Failed to join multicast group\n");
        ret = -1;
        goto exit;
    }
