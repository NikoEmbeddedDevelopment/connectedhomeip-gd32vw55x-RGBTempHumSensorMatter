...
static gd32vw55x_route_entry_t s_route_entries[MAX_RIO_ROUTE];

static gd32vw55x_route_entry_t * find_route_entry(const gd32vw55x_route_entry_t * route_entry)
{
...
static gd32vw55x_route_entry_t * find_empty_route_entry(void)
{
    for (size_t i = 0; i < LWIP_ARRAYSIZE(s_route_entries); i++)
    {
        if (s_route_entries[i].netif == NULL)
...
{
    gd32vw55x_route_entry_t * route = (gd32vw55x_route_entry_t *) arg;

    gd32vw55x_route_table_remove_route_entry(route);
}

gd32vw55x_route_entry_t * gd32vw55x_route_table_add_route_entry(const gd32vw55x_route_entry_t * route_entry)
{
    ...
    gd32vw55x_route_entry_t * entry = find_route_entry(route_entry);
...
int8_t gd32vw55x_route_table_remove_route_entry(gd32vw55x_route_entry_t * route_entry)
{
    ...
    for (gd32vw55x_route_entry_t * moved = route_entry; moved < &s_route_entries[LWIP_ARRAYSIZE(s_route_entries) - 1]; moved++)
    {
        *moved = *(moved + 1);
        if (moved->netif == NULL)
        {
...
static inline bool is_better_route(const gd32vw55x_route_entry_t * lhs, const gd32vw55x_route_entry_t * rhs)
{
    if (rhs == NULL)
    {
        return true;
...
static inline bool route_match(const gd32vw55x_route_entry_t * route, const ip6_addr_t * dest)
{
...
{
    gd32vw55x_route_entry_t * route = NULL;
