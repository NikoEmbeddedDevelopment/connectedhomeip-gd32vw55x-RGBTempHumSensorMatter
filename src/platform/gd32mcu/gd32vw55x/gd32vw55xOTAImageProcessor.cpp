...
#include <platform/gd32mcu/gd32vw55x/gd32vw55xOTAImageProcessor.h>
...
CHIP_ERROR GD32VW55xOTAImageProcessor::PrepareDownload()
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::Finalize()
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::Apply()
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::Abort()
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::ProcessBlock(ByteSpan & block)
{
...
bool GD32VW55xOTAImageProcessor::IsFirstImageRun()
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::ConfirmCurrentImage()
{
    OTARequestorInterface * requestor = chip::GetRequestorInstance();
    if (requestor == nullptr)
    {
...
void GD32VW55xOTAImageProcessor::HandlePrepareDownload(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<GD32VW55xOTAImageProcessor *>(context);
...
void GD32VW55xOTAImageProcessor::HandleFinalize(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<GD32VW55xOTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
void GD32VW55xOTAImageProcessor::HandleAbort(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<GD32VW55xOTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
void GD32VW55xOTAImageProcessor::HandleProcessBlock(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<GD32VW55xOTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
#if defined(CONFIG_PLATFORM_8721D)
    if (imageProcessor->RemainHeader != 0) // Still not yet received full GD32VW55x header
    {
        if (block.size() >= imageProcessor->RemainHeader)
        {
            memcpy(imageProcessor->GD32VW55xHeader + HeaderOffset, block.data(), imageProcessor->RemainHeader);
            imageProcessor->pOtaTgtHdr = (update_ota_target_hdr *) ota_update_malloc(sizeof(update_ota_target_hdr));

            memcpy(imageProcessor->pOtaTgtHdr, imageProcessor->GD32VW55xHeader, 8);                             // Store FwVer, HdrNum
            memcpy(&(imageProcessor->pOtaTgtHdr->FileImgHdr[0].ImgId), imageProcessor->GD32VW55xHeader + 8, 4); // Store OTA id
            memcpy(&(imageProcessor->pOtaTgtHdr->FileImgHdr[0].ImgHdrLen), imageProcessor->GD32VW55xHeader + 12,
        ...
        {
            memcpy(imageProcessor->GD32VW55xHeader + HeaderOffset, block.data(), block.size());
...
void GD32VW55xOTAImageProcessor::HandleApply(intptr_t context)
{
    auto * imageProcessor = reinterpret_cast<GD32VW55xOTAImageProcessor *>(context);
    if (imageProcessor == nullptr)
    {
...
void GD32VW55xOTAImageProcessor::HandleRestart(chip::System::Layer * systemLayer, void * appState)
{
    ota_platform_reset();
}

CHIP_ERROR GD32VW55xOTAImageProcessor::ProcessHeader(ByteSpan & block)
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::SetBlock(ByteSpan & block)
{
...
CHIP_ERROR GD32VW55xOTAImageProcessor::ReleaseBlock()
{
