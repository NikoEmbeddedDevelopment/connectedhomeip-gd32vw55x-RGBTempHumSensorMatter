...
#include <platform/gd32mcu/gd32vw55x/CHIPDevicePlatformConfig.h>
...
{
#if !CONFIG_ENABLE_GD32VW55X_FACTORY_DATA_PROVIDER
...
template <class FlashFactoryData>
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::SignWithDeviceAttestationKey(const ByteSpan & messageToSign, MutableByteSpan & outSignBuffer)
{
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetSpake2pSalt(MutableByteSpan & saltBuf)
{
    ...
    ReturnErrorCodeIf(saltB64Len > sizeof(saltB64), CHIP_ERROR_BUFFER_TOO_SMALL);
    memcpy(saltB64, mFactoryData.spake2_salt.data, saltB64Len);
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetSpake2pVerifier(MutableByteSpan & verifierBuf, size_t & verifierLen)
{
    ...
    ReturnErrorCodeIf(verifierB64Len > sizeof(verifierB64), CHIP_ERROR_BUFFER_TOO_SMALL);
    memcpy(verifierB64, mFactoryData.spake2_verifier.data, verifierB64Len);
    verifierLen = chip::Base64Decode32(verifierB64, verifierB64Len, reinterpret_cast<uint8_t *>(verifierB64));
    ReturnErrorCodeIf(verifierLen > verifierBuf.size(), CHIP_ERROR_BUFFER_TOO_SMALL);
    memcpy(verifierBuf.data(), verifierB64, verifierLen);
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetVendorName(char * buf, size_t bufSize)
{
    ...
    err = CHIP_NO_ERROR;
#else
    err =  GetFactoryDataString(mFactoryData.vendor_name, buf, bufSize);
#endif
    return err;
}
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetProductName(char * buf, size_t bufSize)
{
    ...
    err = CHIP_NO_ERROR;
#else
   err = GetFactoryDataString(mFactoryData.product_name, buf, bufSize);
#endif
    return err;
}
...
#else
    err = GetFactoryDataString(mFactoryData.sn, buf, bufSize);
#endif
    return err;
}
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetHardwareVersionString(char * buf, size_t bufSize)
{
    ...
    err = CHIP_NO_ERROR;
#else
    err = GetFactoryDataString(mFactoryData.hw_ver_str, buf, bufSize);
#endif
return err;
}
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetRotatingDeviceIdUniqueId(MutableByteSpan & uniqueIdSpan)
{
    ...
    err =  CHIP_NO_ERROR;
#endif /* CHIP_ENABLE_ROTATING_DEVICE_ID&&CHIP_DEVICE_CONFIG_ROTATING_DEVICE_ID_UNIQUE_ID */
...
template <class FlashFactoryData>
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::GetDiscoverMode(uint16_t & discovermode)
{

#if !CONFIG_ENABLE_GD32VW55X_FACTORY_DATA_PROVIDER

    discovermode = static_cast<uint16_t>(CONFIG_RENDEZVOUS_MODE);

#else
    VerifyOrReturnError(mFactoryData.dismodePresent, CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
    discovermode = mFactoryData.dis_mode;
#endif
