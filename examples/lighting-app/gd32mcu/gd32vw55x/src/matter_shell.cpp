...
/* ---------------- WiFi shell command (from sensor app) ---------------- */
...
static CHIP_ERROR WiFiConnectHandler(int argc, char ** argv)
{
    ...
    return chip::DeviceLayer::NetworkCommissioning::GD32VW55xWiFiDriver::GetInstance().ConnectWiFiNetwork(
...
{
    // Register WiFi Shell Commands manually (supplement default ones)
...
{
    // If you have UART RX, implement here.
    return 0;
}
...
{
    // uart_puts_noint expects a C-string. If buf isn't null-terminated, this is best-effort.
    // Many existing examples still do this; keep behavior consistent.
