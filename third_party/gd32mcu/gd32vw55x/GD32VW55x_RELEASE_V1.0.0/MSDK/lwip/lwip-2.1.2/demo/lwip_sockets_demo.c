            ...
            if ((getsockopt(sock_fd->fd, SOL_SOCKET, SO_TYPE, (void *)&type, &type_len) < 0)
                ...
                if (type == SOCK_STREAM) {
                    if (sock_fd->mode == SOCKET_SERVER)
                        printf("socket[%d] fd:%d type:TCP-server.\r\n", idx, sock_fd->fd);
                    else if (sock_fd->mode == SOCKET_CLIENT)
                        printf("socket[%d] fd:%d type:TCP-client.\r\n", idx, sock_fd->fd);
                } else if (type == SOCK_DGRAM) {
                    if (sock_fd->mode == SOCKET_SERVER)
                        printf("socket[%d] fd:%d type:UDP-server.\r\n", idx, sock_fd->fd);
                    else if (sock_fd->mode == SOCKET_CLIENT)
                        printf("socket[%d] fd:%d type:UDP-client.\r\n", idx, sock_fd->fd);
                }
            }
        }
    }
