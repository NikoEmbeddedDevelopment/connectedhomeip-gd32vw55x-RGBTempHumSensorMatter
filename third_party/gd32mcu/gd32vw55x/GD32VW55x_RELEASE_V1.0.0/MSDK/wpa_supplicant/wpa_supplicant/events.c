					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	while ((ssid->proto & (WPA_PROTO_RSN | WPA_PROTO_OSEN)) && rsn_ie) {
		...
		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip RSN IE - key mgmt mismatch (%d - %d)", ie.key_mgmt, ssid->key_mgmt);
			break;
		}
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		wpa_drv_suspend_ap(wpa_s, WIFI_VIF_INDEX_SOFTAP_MODE, selected->freq);
		#endif
...
#ifdef CONFIG_SME
#ifndef CONFIG_SME_EXT_AUTH_ONLY
	os_memcpy(wpa_s->sme.prev_bssid, bssid, ETH_ALEN);
	wpa_s->sme.prev_bssid_set = 1;
	wpa_s->sme.last_unprot_disconnect.sec = 0;
#endif /* CONFIG_SME_EXT_AUTH_ONLY */
#endif /* CONFIG_SME */
...
#ifndef CONFIG_NO_PNO_OR_SCHE_SCAN
		if (!wpa_s->pno)
			wpa_supplicant_cancel_sched_scan(wpa_s);
...
#ifndef CONFIG_AP_NO_DFS
	if (wpa_s->ap_iface || wpa_s->ifmsh) {
		wpas_ap_event_dfs_cac_started(wpa_s, radar);
	} else
#endif /* CONFIG_AP_NO_DFS */
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_UNPROT_BEACON:
		...
		break;
#endif /* CONFIG_REMOVE_UNUSED_EVENTS */
	default:
