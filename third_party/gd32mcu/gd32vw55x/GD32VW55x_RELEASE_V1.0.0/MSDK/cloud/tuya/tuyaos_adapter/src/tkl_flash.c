...
#define TUYA_FLASH_UF_START          (RE_IMG_1_END) // 0x3d0000
//KV DATA
#define TUYA_FLASH_KV_DATA_SIZE     (0x10000) //64K
#define TUYA_FLASH_KV_DATA_START    (TUYA_FLASH_UF_START + TUYA_FLASH_UF_SIZE)           // 0x3e0000
//KV KEY
#define TUYA_FLASH_KV_KEY_SIZE      (0x1000) //4K
#define TUYA_FLASH_KV_KEY_START     (TUYA_FLASH_KV_DATA_START + TUYA_FLASH_KV_DATA_SIZE) // 0x3e1000
//KV SWAP
#define TUYA_FLASH_KV_SWAP_SIZE     (0x8000) //32K
#define TUYA_FLASH_KV_SWAP_START    (TUYA_FLASH_KV_KEY_START + TUYA_FLASH_KV_KEY_SIZE)   // 0x3e9000
// KV_PROTECT
#if defined(KV_PROTECTED_ENABLE) && (KV_PROTECTED_ENABLE == 1)
#define TUYA_FLASH_KV_PROTECT_SIZE  (0x1000) //4K
#define TUYA_FLASH_KV_PROTECT_START (TUYA_FLASH_KV_SWAP_START + TUYA_FLASH_KV_SWAP_SIZE) // 0x3ea000
#endif
...
OPERATE_RET tkl_flash_read(UINT32_T addr, UCHAR_T *dst, UINT32_T size)
{
    // --- BEGIN: user implements ---
     if (addr <= RE_END_OFFSET) {
        if (!rom_flash_read(addr, dst, size))
            return OPRT_OK;
    }
...
OPERATE_RET tkl_flash_write(UINT32_T addr, CONST UCHAR_T *src, UINT32_T size)
{
    // --- BEGIN: user implements ---
    if (addr <= RE_END_OFFSET) {
        if (!rom_flash_write(addr, src, size))
            return OPRT_OK;
    }
