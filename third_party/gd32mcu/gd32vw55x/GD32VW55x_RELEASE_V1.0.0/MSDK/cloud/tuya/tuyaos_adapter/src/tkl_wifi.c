...
OPERATE_RET tkl_wifi_set_lp_mode(CONST BOOL_T enable, CONST UCHAR_T dtim)
{
    ...
    if (enable) {
        if (wifi_netlink_ps_mode_set(WIFI_VIF_INDEX_DEFAULT, 1)) {
            return OPRT_COM_ERROR;
        }
    } else {
        if (wifi_netlink_ps_mode_set(WIFI_VIF_INDEX_DEFAULT, 0)) {
            return OPRT_COM_ERROR;
        }
    }
    return OPRT_OK;
...
{
    // --- BEGIN: user implements ---
    if (wifi_management_connect((CHAR_T *)ssid, (CHAR_T *)passwd, false)) {
        return OPRT_COM_ERROR;
    }
...
OPERATE_RET tkl_wifi_station_get_conn_ap_rssi(SCHAR_T *rssi)
{
    ...
    if (macif_vif_sta_rssi_get(WIFI_VIF_INDEX_DEFAULT) == 0)
...
{
    // --- BEGIN: user implements ---
//    INT32_T vif_idx = WIFI_VIF_INDEX_DEFAULT;
    switch (cmd)
    {
