...
OPERATE_RET tkl_semaphore_wait(CONST TKL_SEM_HANDLE handle, UINT_T timeout)
{
    ...
    if (timeout == TKL_SEM_WAIT_FOREVER) {
        ret = sys_sema_down((os_sema_t *)&handle, 0);
    } else {
        ret = sys_sema_down((os_sema_t *)&handle, timeout);
    }
...
OPERATE_RET tkl_semaphore_post(CONST TKL_SEM_HANDLE handle)
{
    ...
    if (__get_CONTROL() == 1) {
        sys_sema_up_from_isr((os_sema_t *)&handle);
    } else if(__get_CONTROL() == 0) {
        sys_sema_up((os_sema_t *)&handle);
    }
...
OPERATE_RET tkl_semaphore_release(CONST TKL_SEM_HANDLE handle)
{
    ...
    sys_sema_free((os_sema_t *)&handle);
    return OPRT_OK;
    // --- END: user implements ---
}
