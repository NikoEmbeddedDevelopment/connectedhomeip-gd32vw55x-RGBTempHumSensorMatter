...
void HAL_Firmware_Persistence_Start(void)
{
    ...
    if (running_idx == IMAGE_0) {
        img_addr = RE_IMG_1_OFFSET;
        max_img_len = RE_END_OFFSET - RE_IMG_1_OFFSET;
    } else {
        img_addr = RE_IMG_0_OFFSET;
        max_img_len = RE_IMG_1_OFFSET - RE_IMG_0_OFFSET;
    }

    res = rom_flash_erase(img_addr, max_img_len);
    if (res < 0)
...
int HAL_Firmware_Persistence_Write(char *buffer, uint32_t length)
{
    ...
    res = rom_flash_write((img_addr + g_firmware_offset), (uint8_t *)buffer, length);
...
#define ALI_LINKIT_THREAD_PRI OS_TASK_PRIORITY(ALICLOUD_TASK_PRIO)
            ...
            int *stack_used)
{
    ...
    task_handle = sys_task_create(NULL, (const uint8_t *)task_name, NULL,
                    stack_size, 0, 0, task_pri, task_function, task_ctx);
    if (task_handle == NULL) {
        sys_mfree(task_ctx);
        return -1;
    }
            ...
            _IN_ awss_wifi_mgmt_frame_cb_t callback)
{
    ...
    //TODO 暂时未实现该功能，不支持在station模式下过滤management frame
    return 0;
}
...
int HAL_Awss_Open_Ap(const char *ssid, const char *passwd, int beacon_interval, int hide)
{
    ...
    if (ret)
        platform_warn("Failed to start AP, check your configuration.\r\n");
    else
